...
public class SecGroupRepository : RepositoryBase<SecGroup, SecGroupDto, SecGroupCreateDto, SecGroupUpdateDto>, ISecGroupRepository
{
    ...
    public async override Task<ParentResponseModel> FindAll()
    {
        try
        {
            var listOfObjects = await RepositoryContext.SecGroups
                ...
                .Select(e => new
                {
                    ...
                    ModulesInGroups = RepositoryContext.SecModules.Where(e=>(bool)e.IsTaken)
                        .Select(m => new ModulesInGroup
                        {
                            ModuleId = m.Id,
                            ModuleName = m.ModuleName,
                            IsAssigned = m.SecModuleGroups
                                .Any(mg => mg.GroupCode == e.Id && mg.ModuleCode == m.Id)
                        })
                        .ToList()
                })
                .ToListAsync();
        ...
        {
            new Dictionary<string, string> { { "field", "groupName" }, { "header", "«”„ «·„Ã„Ê⁄…" } }
        };

            //  Create a unique dictionary for column indexing
            var moduleFields = new Dictionary<int, string>();
            int moduleIndex = 1;

            foreach (var group in listOfObjects)
            {
                foreach (var module in group.ModulesInGroups)
                {
                    if (!moduleFields.ContainsKey(module.ModuleId)) // Prevent duplicates
                    {
                        string moduleField = $"module{moduleIndex}";
                        moduleFields[module.ModuleId] = moduleField;
                ...
                var row = new Dictionary<string, object>
            {
                ...
                foreach (var module in group.ModulesInGroups)
                {
                    if (moduleFields.TryGetValue(module.ModuleId, out string moduleField))
                    {
                        row[moduleField] = module.IsAssigned;
                        row[$"moduleId_{moduleIndexIds}"] = module.ModuleId;
                    }
                    moduleIndexIds++;
                }
    ...
    public async override Task<ParentResponseModel> Create(SecGroupCreateDto entityCreate)
    {
        try
        {
            ...
            var entity = new SecGroup()
            {
                ...
                SecModuleGroups = entityCreate.ModuleIds.Select(moduleCode => new SecModuleGroup
                {
    ...
    public override async Task<ParentResponseModel> Update(SecGroupUpdateDto secGroupUpdateDto)
    {
        try
        {
            var entity = await RepositoryContext.SecGroups.Include(e => e.SecModuleGroups).AsTracking().FirstOrDefaultAsync(t => t.Id == secGroupUpdateDto.Id);
            ...
            if (secGroupUpdateDto.ModuleIds!=null)
            {
                entity.SecModuleGroups = secGroupUpdateDto.ModuleIds.Select(e => new SecModuleGroup
                {
                    ModuleCode = e,
                    GroupCode = secGroupUpdateDto.Id
                }).ToList();
            }
        ...
        {
            _logger.logErrorWithException(ex, $"{typeof(Group).Name} ===> Update ");
            return new ParentResponseModel()
            {
                ErrorCode = ErrorCatalog.DataBaseFauiler,
                IsDone = false,
